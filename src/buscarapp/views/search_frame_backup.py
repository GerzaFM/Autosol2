...
class SearchFrame:
    ...
    def __init__(self, parent, on_search_callback: Optional[Callable] = None, 
        ...
        # Crear frame principal (NO LABELFRAME)
    ...
    def enable_controls(self, enabled: bool = True):
        ...
        for widget in entry_widgets:
            try:
                ...
                # Ignorar widgets que no soportan 'state'
                pass
        
        # Manejar checkboxes por separado si es necesario
        try:
            if hasattr(self.solo_cargado_check, 'config'):
                self.solo_cargado_check.config(state=state)
        except Exception:
            pass
            
        try:
            if hasattr(self.solo_pagado_check, 'config'):
                self.solo_pagado_check.config(state=state)
        except Exception:
            pass
