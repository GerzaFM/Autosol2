...
class FormularioMaestroDesign(tb.Window):
    ...
    def controles_barra_superior(self):
        self.labelTitulo = tb.Label(self.barra_superior, text="Autodidacta", font=("Segoe UI", 15, "bold"), bootstyle="inverse-secondary")
        self.labelTitulo.pack(side=LEFT, padx=10, pady=10)

        self.labelInfo = tb.Label(self.barra_superior, text="Administraciión", font=("Segoe UI", 10), bootstyle="inverse-secondary")
    ...
    def on_menu_click(self, name):
        for widget in self.cuerpo_principal.winfo_children():
            widget.destroy()
        tb.Label(self.cuerpo_principal, text=f"Seleccionado: {name}", font=("Segoe UI", 18), bootstyle="inverse-dark").pack(pady=40)
